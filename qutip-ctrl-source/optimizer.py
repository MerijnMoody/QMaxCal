...
import pulsegen as pulsegen
import dump as qtrldump
...
class Optimizer(object):
    ...
    def fid_err_func_wrapper(self, *args):
        ...
        amps = self._get_ctrl_amps(args[0][:-1].copy())
        self.dynamics.update_ctrl_amps(amps)
        
        #raise TypeError(str(args))
        self.dynamics.lam = args[0][-1]
    ...
    def fid_err_grad_wrapper(self, *args):
        ...
        return np.append(grad.flatten(),fid_comp.get_fid_err())
    ...
    def run_optimization(self, term_conds=None):
        ...
        try:
            optim_var_vals, fid, res_dict = fmin_l_bfgs_b(
                self.fid_err_func_wrapper,
                np.append(self.optim_var_vals,dyn.lam),
                fprime=fprime,
            ...
            amps = self._get_ctrl_amps(optim_var_vals[:-1])
            dyn.update_ctrl_amps(amps)
            dyn.lam = optim_var_vals[-1]
