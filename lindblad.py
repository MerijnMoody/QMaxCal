...
class LindBladEvolve(object):
    ...
    def _compute_fid_err_grad(self):
        ...
        for j in range(n_ctrls):
            for k in range(n_ts):
                ...
                entg = (_grad_ent(vector_to_operator(self.fwd_evo[k+1]),
                                  self.ref_evo[k+1],
                                  vector_to_operator(evo_grad))
                        + vector_to_operator(evo_grad).tr()
                        )
    ...
    def ascent(self, i_num, eps, c, fid_tar):
        ...
        for i in range(i_num):
            ...
            #ent_grad = self.approximated_ent_grad(self.ctrls)
            ...
            for k in range(self.n_ts):
                for j in range(self.n_ctrls):
                    self.ctrls[k][j] -= ent_grad[k] + abs(self.lam) * fid_grad[k] + c * fid_error * fid_grad[k]
