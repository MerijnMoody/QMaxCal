...
class LindBladEvolve(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.forbidden_state_idx = None  # Index of forbidden state
        self.forbidden_penalty = torch.tensor(0.0)  # Penalty strength
        self.forbidden_occupation = torch.tensor(0.0)  # Current occupation
        self.forbidden_state = None  # The forbidden state vector
    ...
    def _compute_propagator_ref(self):
        ...
        # Fix: Initialize propagators with correct dimension
        propagators = torch.zeros(t_steps, self.dim, self.dim, dtype=torch.complex128)
    ...
    def _probability_distribution_estimate(self, ref: bool, max_iters: int) -> Tuple[List[torch.Tensor], Dict, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        forbidden_occupation_sum = torch.zeros(1, dtype=torch.float64)
        while num_samples < max_iters:
            ...
            # Modified population tracking to store all states
            populations = []
            for state in result.state_trajectory:
                ...
                state_probs = [(state[i]*state[i].conj()).real.detach().item() for i in range(self.dim)]
                populations.append(state_probs)
            all_trajectories.append(populations)

            # Calculate forbidden state occupation if specified
            if self.forbidden_state is not None:
                for state in result.state_trajectory:
                    # Calculate |⟨ψf|ψj⟩|²
                    overlap = (self.forbidden_state.conj() @ state).abs()**2
                    forbidden_occupation_sum += overlap * self.dt
        ...
        if not ref:
            self.iteration_trajectories.append({
                ...
                'std': std_trajectory,
                'time': self.time_list,
                'states': result.state_trajectory  # Store full state information
            })
        
        forbidden_occupation_avg = forbidden_occupation_sum / (max_iters * len(self.time_list))
        return [], probability_distribution, energy_avg, fidelity_avg, forbidden_occupation_avg
    ...
    def _get_err(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        """Calculate error metrics for current controls"""
        print("\nEntering _get_err")
        # Update to match actual return values
        evolution, p1, energy_average, fidelity_avg, forbidden_occupation = self._probability_distribution_estimate(
            ref=False, max_iters=200
        )
        ...
        if self.ref_evo is None:
            ref_evolution, p2, energy_ref_avg, _, _ = self._probability_distribution_estimate(
        ...
        self.forbidden_occupation = forbidden_occupation
        return relative_entropy, energy_average, fidelity_avg, forbidden_occupation
    ...
    def _loss_function(self, args: List) -> torch.Tensor:
        ...
        # Calculate metrics
        relative_entropy, energy_average, fidelity_avg, forbidden_occupation = self._get_err()
        ...
        print(f"Relative entropy: {relative_entropy.item()}")
        print(f"Fidelity: {fidelity_avg.item()}")
        print(f"forbidden_occupation: {forbidden_occupation.item()}")

        # Simple loss with C2 cost function
        loss = relative_entropy + self.lam * fidelity_avg*3 + self.lam2*forbidden_occupation*1000
        return loss
